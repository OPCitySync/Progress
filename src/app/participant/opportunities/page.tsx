...
export default async function OpportunitiesPage() {
  ...
  const recommended = await recommendedOpportunities(session.sub, city?.id)
    ...
    <>
      <Card>
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <h1 className="skeuo-page-title font-display text-2xl font-semibold text-ink-900">Open opportunities</h1>
            <p className="mt-1 text-sm text-ink-500">
              {city ? `Approved opportunities in ${city.name}. Pick a shift to sign up.` : 'Add a city network before browsing opportunities.'}
            </p>
          </div>
        </div>
        ...
        <div className="mt-8">
          {cards.length === 0 ? (
            <EmptyState
              title={city ? 'No open opportunities right now' : 'Choose a city network first'}
              body={city ? 'Check back soon — issuer organizations publish new opportunities regularly.' : 'Use the + button on the city rail to add Berkeley or Mexico City.'}
            />
          ) : (
            <div className="grid gap-4 md:grid-cols-2">
              {cards.map(({ card, org }) => (
