...
export default async function LabOrganizationProfilePage({ params }: { params: { slug: string } }) {
  ...
  const [opportunities, publishedResources, publicRoleRows] = await Promise.all([
    getOpenOpportunities(org.id),
    getPublishedOrganizationResources({ orgIds: [org.id], destination: 'profile' }),
    db.select({ task: tasks, intake: onboardingIntakes }).from(tasks).innerJoin(onboardingIntakes, eq(onboardingIntakes.taskId, tasks.id)).where(and(
      eq(tasks.orgId, org.id), eq(tasks.status, 'open'), eq(tasks.isOnboarding, 0),
      eq(onboardingIntakes.applicationPublic, 1),
      city ? eq(tasks.cityId, city.id) : undefined,
  ...
  return <main className={styles.app}>
    ...
    <section className={styles.detailLayout}>
      ...
      <section className={styles.primaryColumn}>
        ...
        {publicRoleRows.length ? <section id="volunteer-roles" className={`${styles.labPanel} ${styles.labStack}`}><div><p className={styles.eyebrow}>Volunteer roles</p><h2>Find a role with {org.name}</h2><p>Select Apply to share your City/Sync profile for organization review. Some roles may include a few additional questions.</p></div><div className={styles.labChoiceList}>{publicRoleRows.map(({ task }) => {
          return <article className={styles.labChoice} key={task.id}><div><p><strong>{task.title}</strong></p><small>{task.description || 'A volunteer role open to Civic-Participants.'} · Organization approval required{scheduledRoleIds.has(task.id)?' · Public shifts are available':''}</small></div><Link className={styles.labLinkButton} href={`/aesthetic-lab/opportunities/${task.id}#application`}>Apply <ArrowUpRight size={14} /></Link></article>
        })}</div></section> : null}
