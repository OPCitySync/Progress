...
import { ArrowLeft, ArrowUpRight, Building2, CheckCircle2, Heart, MapPin, UsersRound } from 'lucide-react'
...
export default async function LabOrganizationProfilePage({ params }: { params: { slug: string } }) {
  ...
  const opportunities = await getOpenOpportunities(org.id)
  const causes = profile?.causes.length ? profile.causes.join(' · ') : 'Community organization'
  return <main className={styles.app}>
    <LabHeader {...headerProps} />
    <section className={styles.detailLayout}><aside className={styles.leftRail}><section className={styles.cityCard}><Building2 size={20} /><h2>{org.name}</h2><p>{causes}</p><Link href="/aesthetic-lab/organizations"><ArrowLeft size={14} /> Discover organizations</Link></section></aside><section className={styles.primaryColumn}><div className={styles.pageIntro}><p className={styles.eyebrow}>Approved local organization</p><h1>{org.name} <CheckCircle2 size={19} /></h1><p>{profile?.tagline || org.description || 'A City/Sync organization helping its local community.'}</p></div><section className={`${styles.labPanel} ${styles.labStack}`}><div><p className={styles.eyebrow}>About</p><h2>What they do</h2></div><p>{profile?.mission || org.description || 'This organization has not added its public mission yet.'}</p><div className={styles.labChoiceList}><div className={styles.labChoice}><p><MapPin size={15} /> <strong>Location</strong><small>{profile?.location || city?.name || 'Local City/Sync network'}</small></p><p><Heart size={15} /> <strong>Causes</strong><small>{causes}</small></p></div>{profile?.contactEmail ? <div className={styles.labChoice}><p><strong>Contact</strong><small>{profile.contactEmail}</small></p>{profile.website ? <a className={styles.labLinkButton} href={profile.website} target="_blank" rel="noreferrer">Website <ArrowUpRight size={14} /></a> : null}</div> : null}</div></section><section className={`${styles.labPanel} ${styles.labStack}`}><div><p className={styles.eyebrow}>Ways to participate</p><h2>Open opportunities</h2></div>{opportunities.length ? opportunities.map((opportunity) => <article className={styles.labChoice} key={opportunity.id}><div><p><strong>{opportunity.title}</strong></p><small>{opportunity.location || 'Location TBD'} · {opportunity.totalOpenSlots} spot{opportunity.totalOpenSlots === 1 ? '' : 's'} open</small></div><Link className={styles.labLinkButton} href={`/aesthetic-lab/opportunities/${opportunity.id}`}>View sessions <ArrowUpRight size={14} /></Link></article>) : <p className={styles.emptyCopy}>No open opportunities are scheduled right now.</p>}</section></section><aside className={styles.rightRail}><section className={styles.discoveryAside}><UsersRound size={20} /><p className={styles.eyebrow}>Showing up</p><h2>Start with a clear next step.</h2><p>Every listed opportunity includes current capacity and location details.</p><Link href="/aesthetic-lab/opportunities">Browse opportunities</Link></section></aside></section>
  </main>
}
