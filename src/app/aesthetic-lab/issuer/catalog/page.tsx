      ...
      <div className={styles.issuerLayout}>
        ...
        <section className={styles.issuerMain} aria-label="Opportunity Catalog">
          <section className={styles.issuerPageHero}>
            ...
            <Link href="/aesthetic-lab/issuer/new" className={styles.issuerPrimaryAction}><Plus size={18} /> New opportunity</Link>
          </section>

          <section className={styles.onboardingWorkspaceCard}>
            <div className={styles.onboardingWorkspaceHeading}><span><Repeat2 size={20} /></span><div><p className={styles.eyebrow}>Recurring onboarding</p><h2>{onboarding?.task.title ?? 'No onboarding session yet'}</h2><p>{onboarding ? 'This is the first step for prospective City Members.' : 'Create a recurring orientation so new participants have a clear first step.'}</p></div><Link href={onboarding ? `/aesthetic-lab/issuer/opportunities/${onboarding.task.id}` : '/aesthetic-lab/issuer/onboarding'}><Edit3 size={15} /> {onboarding ? 'Manage session' : 'Create session'}</Link></div>
            {onboarding ? <>
              ...
              <div className={styles.upcomingSessions}>
                {onboarding.sessions.slice(0, 3).map(({ shift, taken }) => <article key={shift.id}><CalendarDays size={16} /><b>{shift.startsAt ? new Date(shift.startsAt).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' }).toUpperCase() : 'TBD'}</b><span>{shift.startsAt ? new Date(shift.startsAt).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' }) : 'Time TBD'}</span><em>{taken} of {shift.capacity} reserved</em><Link href={`/aesthetic-lab/issuer/opportunities/${onboarding.task.id}`} aria-label={`Manage ${onboarding.task.title}`}><MoreHorizontal size={19} /></Link></article>)}
              </div>
            </> : null}
          </section>
          ...
          <section className={styles.catalogWorkspaceCard}>
            <div className={styles.issuerPanelHeading}>
              ...
              <Link href="/aesthetic-lab/issuer/new" className={styles.issuerTextButton}><Plus size={15} /> New opportunity</Link>
            </div>
            <div className={styles.templateGrid}>
              {taskShifts.length === 0 ? <p className={styles.emptyCopy}>Your opportunities will appear here once you publish them.</p> : taskShifts.map(({ task, sessions }) => {
                ...
                return <article key={task.id} className={styles.templateCard}>
                  ...
                  <div className={styles.templateTitle}><span>{isOnboarding ? 'Onboarding' : task.status === 'open' ? 'Volunteer shift' : task.status}</span><Link href={`/aesthetic-lab/issuer/opportunities/${task.id}`} aria-label={`Manage ${task.title}`}><MoreHorizontal size={19} /></Link></div>
                  ...
                  <div className={styles.templateActions}><Link href={`/aesthetic-lab/issuer/opportunities/${task.id}`}><Copy size={14} /> Manage</Link><Link href={`/aesthetic-lab/issuer/opportunities/${task.id}`}><CalendarDays size={14} /> {card?.openShiftCount ?? 0} sessions</Link></div>
                </article>
              })}
            </div>
          </section>
          ...
          <section className={styles.catalogFootnote}>
            ...
            <Link href="/aesthetic-lab/issuer/new">Publish another opportunity <ArrowUpRight size={15} /></Link>
          </section>
        </section>
      </div>
    </main>
