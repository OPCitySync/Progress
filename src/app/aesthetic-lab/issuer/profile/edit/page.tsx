...
export default async function EditLabIssuerProfilePage({ searchParams }: { searchParams: { ok?: string; error?: string } }) {
  ...
  return <main className={styles.app}><LabHeader activeSection="issuer-profile" workspace="issuer" session={session} city={city} cities={cities} contexts={contexts} /><section className={styles.detailLayout}><aside className={styles.leftRail}><section className={styles.cityCard}><Edit3 size={20} /><h2>Edit public profile</h2><p>Give prospective volunteers current, helpful information.</p><Link href="/aesthetic-lab/issuer/profile"><ArrowLeft size={14} /> Public profile</Link></section></aside><section className={styles.primaryColumn}><div className={styles.pageIntro}><p className={styles.eyebrow}>Public profile</p><h1>Tell your organization’s story.</h1><p>These details appear to people discovering your organization in their local city network.</p></div><LabNotice hidden ok={searchParams.ok} error={searchParams.error} /><section className={styles.labPanel}><IssuerProfileForm profile={profile} tasks={tasks} /></section></section></section></main>
}
