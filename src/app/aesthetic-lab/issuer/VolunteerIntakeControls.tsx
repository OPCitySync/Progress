...
export function ParticipantProfileApplication({taskId,title,status,blocked,roles=[]}:{taskId:string;title:string;status:string|null;blocked?:string|null;roles?:RoleInterest[]}){
  const[open,setOpen]=useState(false)
  return <section id="application" className={base.onboardingProcessCard+' '+s.surface}><div className={s.toolbar}><div><p className={s.eyebrow}>Volunteer application</p><h2>{status==='approved'?'Your volunteer application is approved':status==='submitted'?'Your application is in review':status==='not_approved'?'Your application was not approved':'Apply with your City/Sync profile'}</h2></div>{!status&&!blocked?<button className={s.button} type="button" onClick={()=>setOpen(true)}>Apply<ArrowRight size={14}/></button>:null}</div><p className={s.hint}>{blocked??(status==='approved'?'Explore the organization’s opportunities and complete any required onboarding before signing up.':status==='submitted'?'The organization is reviewing the profile you already use on City/Sync.':status==='not_approved'?'Contact the organization if you would like to ask about its decision.':'No questionnaire is required. Choose what interests you and share your City/Sync profile for organization review.')}</p>
    {open?<ParticipantProfileApplicationDialog taskId={taskId} title={title} roles={roles} onClose={()=>setOpen(false)}/>:null}
  </section>
}
...
function ParticipantProfileApplicationDialog({taskId,title,roles,onClose}:{taskId:string;title:string;roles:RoleInterest[];onClose:()=>void}){
  const[selected,setSelected]=useState<string[]>([]),[flexible,setFlexible]=useState(false)
  const{save,pending,error,setError}=useIntakeSave(onClose,true)
  ...
  return <IntakeModal title="Apply to Volunteer" onClose={onClose} busy={pending}><form className={s.form} onSubmit={e=>{e.preventDefault();submit()}}><div className={s.preserve}><b>{title}</b><p>The organization will review your existing City/Sync profile. You do not need to complete a separate questionnaire.</p></div><RoleInterestFields roles={roles} selected={selected} onSelected={setSelected} flexible={flexible} onFlexible={setFlexible}/><IntakeError error={error}/><footer className={s.footer}><button type="button" className={s.button} disabled={pending} onClick={onClose}>Cancel</button><button type="submit" className={s.button+' '+s.primary} disabled={pending}>{pending?'Sending…':'Submit Application'}</button></footer></form></IntakeModal>
}
