...
export function InviteOnboardingVolunteersButton({sessions}:{sessions:SessionOption[]}){
  ...
  return <>
    <button type="button" className={`${prototypeStyles.catalogWorkspaceAction} ${prototypeStyles.opportunityWorkspaceButton}`} disabled={!hasAvailable} onClick={openPicker}>{hasAvailable?<><UsersRound size={14}/>Invite Volunteers</>:<><Check size={14}/>Invitations Sent</>}</button>
    {open?<IntakeModal title="Invite Volunteers" onClose={()=>setOpen(false)} busy={pending}>
      <form className={intakeStyles.form} onSubmit={event=>{event.preventDefault();send()}}>
        ...
        <div className={styles.inviteVolunteerToolbar}><b>{selected.length} selected</b><button type="button" className={styles.button} disabled={pending} onClick={()=>setSelected(selected.length===available.length?[]:available.map(candidate=>candidate.id))}>{selected.length===available.length?'Clear selected':'Select all available'}</button></div>
        ...
        <footer className={intakeStyles.footer}><button type="button" className={intakeStyles.button} disabled={pending} onClick={()=>setOpen(false)}>Cancel</button><button type="submit" className={intakeStyles.button+' '+intakeStyles.primary} disabled={pending||!selected.length}><Send size={14}/>{pending?'Sending…':`Send Invitation${selected.length===1?'':'s'}`}</button></footer>
      </form>
    </IntakeModal>:null}
  </>
}
