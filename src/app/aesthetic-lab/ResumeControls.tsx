...
export function ResumeControls({ token, isPublic }: { token: string | null; isPublic: boolean }) {
  ...
  return <div className={styles.resumeActions}>
    <form action={setResumePublicAction}><input type="hidden" name="public" value={isPublic ? 'false' : 'true'} /><input type="hidden" name="redirectTo" value="/aesthetic-lab/history" /><button type="submit">{isPublic ? <EyeOff size={16} /> : <Eye size={16} />}{isPublic ? 'Make private' : 'Make shareable'}</button></form>
