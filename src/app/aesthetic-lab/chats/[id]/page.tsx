...
export default async function ParticipantEventChatPage({ params, searchParams }: { params: { id: string }; searchParams: { ok?: string; error?: string } }) {
  ...
  return <main className={styles.app}>
    ...
    <section className={styles.detailLayout}><aside className={styles.leftRail}><section className={styles.cityCard}><MessageCircle size={19} /><h2>Event chat</h2><p>Shared only with people signed up for the same event.</p><Link href="/aesthetic-lab/messages">Return to messages</Link></section></aside><section className={styles.primaryColumn}>
      <Link className={styles.volunteerProfileBack} href="/aesthetic-lab/messages"><ArrowLeft size={15} /> Back to messages</Link>
