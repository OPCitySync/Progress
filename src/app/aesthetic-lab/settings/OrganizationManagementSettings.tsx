...
function RoleInviteControl({ role, inviteCode, redirectTo }: { role: OrganizationRole; inviteCode?: string; redirectTo: string }) {
  return (
...
function RoleCard({ role, canManage, inviteCode, redirectTo }: { role: OrganizationRole; canManage: boolean; inviteCode?: string; redirectTo: string }) {
  ...
  return (
    <article className={`${styles.manageRoleCard} ${role.isOwnerRole ? styles.manageOwnerRole : ''}`}>
      <div className={styles.manageRoleHeading}>
        ...
        {canManage ? <RoleInviteControl role={role} inviteCode={inviteCode} redirectTo={redirectTo} /> : null}
      </div>
...
function CreateRoleCard({ redirectTo }: { redirectTo: string }) {
  return (
...
export async function OrganizationManagementSettings({
  ...
  inviteRoleId,
  section = 'settings',
  redirectTo = '/aesthetic-lab/settings',
}: {
  ...
  const [roles, delegations, isOwner, activity] = await Promise.all([
    section === 'staff' ? listOrganizationRoles(organization.id) : Promise.resolve([]),
    section === 'staff' ? listOrganizationDelegations(organization.id) : Promise.resolve([]),
    activeSessionIsOrganizationOwner(session),
    section === 'settings' ? listOrganizationActivity(organization.id) : Promise.resolve([]),
  ])
  ...
  return (
    <>
      {section === 'settings' ? (isOwner ? <IssuerOrganizationIdentityForm organizationId={organization.id} organizationName={organization.name} profile={profile} /> : (
      ...
      {section === 'staff' ? <><section id="staff-access" className={`${styles.manageOrganizationCard} ${styles.paletteTreatmentCard}`}>
        <div className={`${styles.manageSectionHeading} ${styles.paletteTreatmentHeader}`}><div><p className={styles.eyebrow}>Staff access</p></div><UsersRound size={19} /></div>
        <div className={`${styles.managePeopleList} ${styles.paletteTreatmentBody}`}>
          ...
          {members.map(({ delegation, user, role }) => {
            ...
            return <details key={delegation.id} className={styles.managePersonDetails}>
              ...
              <div><p><b>Role:</b> {role?.name ?? delegation.role}</p><ul>{memberPowers.map((permission) => <li key={permission.key}><Check size={13} /> {permission.label}</li>)}</ul>{isOwner && delegation.status === 'active' ? <form action={revokeOrganizationDelegationAction}><input type="hidden" name="redirectTo" value={redirectTo} /><input type="hidden" name="delegationId" value={delegation.id} /><button type="submit">Revoke access</button></form> : null}</div>
            </details>
          })}
        </div>
      </section>

      <section className={`${styles.manageRolesSection} ${styles.paletteTreatmentCard}`}>
        <div className={`${styles.manageSectionHeading} ${styles.paletteTreatmentHeader}`}><div><p className={styles.eyebrow}>Roles &amp; functional powers</p></div><ShieldCheck size={19} /></div>
        <div className={`${styles.manageRoleGrid} ${styles.paletteTreatmentBody}`}>
          {roles.map((role) => <RoleCard key={role.id} role={role} canManage={isOwner} inviteCode={inviteRoleId === role.id ? inviteCode : undefined} redirectTo={redirectTo} />)}
          {isOwner ? <CreateRoleCard redirectTo={redirectTo} /> : null}
        </div>
      </section></> : null}

      {section === 'settings' ? <IssuerActivityFeed activity={activity} profile={profile} /> : null}
    </>
  )
}
