  ...
  return (
    <div>
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="font-display text-3xl font-semibold text-ink-900">{data.name}</h1>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard label="Civic credits earned" value={data.totals.credits} />
      </div>
        ...
        <Card className="divide-y divide-ink-100 p-0">
          {data.contributions.map((c, i) => (
            <div key={i} className="flex flex-wrap items-center justify-between gap-3 px-6 py-4">
              ...
              <span className="text-sm font-semibold text-emerald-600">+{c.credits}</span>
            </div>
          ))}
        </Card>
      )}
